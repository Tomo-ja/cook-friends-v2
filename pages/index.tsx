import type { GetServerSideProps, NextPage } from "next";
import Head from "next/head";
import { useState, useEffect } from "react";

import { Alert } from '../components'
import { SearchKeywordSection, HeroSection, RecipesSectionHome, StyledHome } from '../components/Home'
import { StyledMainContent, StyledSubContent} from '../styles'

import { SearchBarSection } from "../components/";

import { User, RandomRecipes, RecipeMinimize, AlertInfo } from "../helpers/typesLibrary";
import appAxios, { spoonacularApiAxios } from "../constants/axiosBase";
import { popupKeywords, getUserFromCookie, getExpiringFood } from "../helpers/functions";
import { getRecipesByIdsSimultaneously } from "../helpers/service";
import { randomRecipeData } from "../sampleApiData";

type Props = {
	user: User | null;
	expireFoods: string[];
	keywords: string[];
	randomRecipes: RecipeMinimize[];
	isFakeData: AlertInfo | null
};

const randomRecipeTags = ["main course", "side dish", "appetizer"];

const Home: NextPage<Props> = ({ user, expireFoods, keywords, randomRecipes, isFakeData}: Props) => {

	const [favoriteRecipes, setFavoriteRecipes] = useState<RecipeMinimize[]>([]);
	const [alert, setAlert] = useState<AlertInfo | null>(isFakeData)

	useEffect(() => {
		if (user === null) return

		const fetchFavRecipes = async () => {
			const favRecipeIds = user.favoriterecipe.length > 3 ? [...user.favoriterecipe].slice(-3) : user.favoriterecipe

			try {
				const recipes = await getRecipesByIdsSimultaneously(favRecipeIds)
				setFavoriteRecipes(recipes)
			} catch {
				setFavoriteRecipes([])
			}
		}

		fetchFavRecipes()
	}, [])

	return (
		<StyledHome>
			<Head>
				<title>Home | Cookit</title>
				<meta name='description' content='Generated by create next app' />
				<link rel='icon' href='/favicon.ico' />
			</Head>
			<SearchBarSection />
			<StyledMainContent>
				<HeroSection randomRecipes={randomRecipes} />

				{favoriteRecipes.length > 0 && (
					<RecipesSectionHome
						title='Favorite Recipes'
						displayRecipes={favoriteRecipes}
						isFavorite={true}
					/>
				)}

			</StyledMainContent>
			<StyledSubContent isOpen={false}>
				{expireFoods.length > 0 && (
					<>
						<h3>Expiring Ingredients</h3>
						<SearchKeywordSection keywords={expireFoods} />
					</>
				)}
				<h3>What Is In Your Mind</h3>
				<SearchKeywordSection keywords={keywords} />
			</StyledSubContent>
			{alert && 
				<Alert isError={alert.isError} message={alert.message} setAlert={setAlert} />
			}
		</StyledHome>
	);
};

export default Home;

export const getServerSideProps: GetServerSideProps = async ({ req }) => {
	const user: User | null = getUserFromCookie(req.headers.cookie)

	let expireFoods: string[] = []
	const keywords = popupKeywords()
	const randomRecipes: RecipeMinimize[] = []
	let isFakeData: AlertInfo | null = null

	if(user) {
		const fridgeData = await appAxios.post("/api/fridge/show", {
			user_id: user.id,
		})
		expireFoods = getExpiringFood(fridgeData.data)
	}

	// Get recipes for slide show in hero section
	try{
		const allRes = await Promise.all(
			randomRecipeTags.map(async (tag) => {
				const response = await spoonacularApiAxios.get("/recipes/random", {
					params: { number: 1, tags: tag }
				})
				return response.data as RandomRecipes;
			})
		)
		allRes.forEach((recipe) => {
			randomRecipes.push(recipe.recipes[0])
		});
	}catch{
		console.error('fake recipes at random recipes')
		isFakeData = {isError: true, message:'Reached Api call Limitation. Displaying Fake Data'}
		randomRecipeData.forEach(recipe => {
			randomRecipes.push(recipe.recipes[0])
		})
	}

	return {
		props: {
			user,
			expireFoods,
			keywords,
			randomRecipes,
			isFakeData
		}
	}

}
